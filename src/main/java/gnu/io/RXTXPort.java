...
public class RXTXPort extends SerialPort
{
    ...
    */
    ...
    public void close()
    {
        ...
        try {
            while( !IOLockedMutex.writeLock().tryLock(500, TimeUnit.MILLISECONDS) )
                {
                    if( debug )
                        z.reportln("close(): IO is locked " + IOLockedMutex.getReadLockCount());
                }
	...
	/**
        ...
        public void write( int b ) throws IOException
        {
            ...
            if( debug ) {
                z.reportln("write(int): 1 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
            }
            ...
            try {
                ...
                if (debug_write)
                    z.reportln( "\nLeaving RXTXPort:SerialOutputStream:write( int )");
            } finally {
                IOLockedMutex.readLock().unlock();
            }
            if( debug ) {
                z.reportln("write(int): 2 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
            }
        }
	/**
         *  @param b[]
        ...
        public void write( byte b[] ) throws IOException
        {
            ...
            if( debug ) {
                z.reportln("write(byte): 1 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
            }
            ...
            try {
                ...
                if (debug_write)
                    z.reportln( "\nLeaving RXTXPort:SerialOutputStream:write(" +b.length  +")");
            } finally {
                IOLockedMutex.readLock().unlock();
            }
            if( debug ) {
                z.reportln("write(byte): 2 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
            }
	...
	/**
                ...
                throws IOException
        {
            ...
            if( debug ) {
                z.reportln("IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
            }
            ...
            try
                {
                    ...
                    if( debug_write )
                        z.reportln( "\nLeaving RXTXPort:SerialOutputStream:write(" + send.length + " " + off + " " + len + " " +") "  /*+ new String(send)*/ );
                } finally {
                IOLockedMutex.readLock().unlock();
            }
	...
	/**
        ...
        public void flush() throws IOException
        {
            ...
            if( debug ) {
                z.reportln("flush(): 1 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
            }
            IOLockedMutex.readLock().lock();
            try
                {
            ...
            if( debug ) {
                z.reportln("flush():  IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
            }
        }
    }
	...
	/**
        ...
        public synchronized int read() throws IOException
        {
            ...
            if( debug ) {
                z.reportln("read(): 1 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
            }
            IOLockedMutex.readLock().lock();
            try {
            ...
            finally
                {
                    IOLockedMutex.readLock().unlock();
                    if( debug ) {
                        z.reportln("read(): 0 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
                    }
                }
	...
	/**
        ...
        public synchronized int read( byte b[] ) throws IOException
        {
            ...
            if( debug ) {
                z.reportln("read(byte): 1 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
            }
            IOLockedMutex.readLock().lock();
            try
                {
            ...
            finally
                {
                    IOLockedMutex.readLock().unlock();
                    if( debug ) {
                        z.reportln("read(byte): 2 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
                    }
                }
        }
        /*
	...
	*/
                ...
                throws IOException
        {
            ...
            if( debug ) {
                z.reportln("read(byte, off, len, byte[]): 1 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
            }
            IOLockedMutex.readLock().lock();
            try
                {
            ...
            finally
                {
                    IOLockedMutex.readLock().unlock();
                    if( debug ) {
                        z.reportln("read(byte, off, len, byte[]): 2 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
                    }
                }
        }
	/**
        ...
        public synchronized int available() throws IOException
        {
            ...
            if( debug ) {
                z.reportln("available(): 1 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
            }
            IOLockedMutex.readLock().lock();
            try
                {
            ...
            finally
                {
                    IOLockedMutex.readLock().unlock();
                    if( debug ) {
                        z.reportln("available(): 2 IOLockedMutex.getReadLockCount(): " + IOLockedMutex.getReadLockCount());
                    }
                }
        }
    }
	...
	/**
        ...
        public void run()
        {
            ...
            eis = 0;
            if (debug)
                z.reportln( "eventLoop() returned, eis is no longer valid"); 
        }
