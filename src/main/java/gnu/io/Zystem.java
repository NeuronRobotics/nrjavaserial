...
public class Zystem {
	...
	static {
		/*
		 * The rxtxZystem library uses Python code and is not included with RXTX. A
		 * seperate library will be released to avoid potential license conflicts.
		 * 
	...
	/**
	 * Constructor. Mode is taken from the java system property "gnu.io.log.mode".
	...
	public void startLogger(String t) throws UnSupportedLoggerException {
		target = t;
		/*
		 * if ( mode == NET_MODE ) { nativeNetInit( ); } if ( nativeInit( ) ) { throw
		 * new UnSupportedLoggerException( "Port initializion failed" ); }
		 */
		return;
	}
