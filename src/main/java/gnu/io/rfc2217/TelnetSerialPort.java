 ...
 * <a href="http://tools.ietf.org/html/rfc2217">RFC 2217</a> serial-over-Telnet
 * protocol as as {@link SerialPort}.
 *
 * 
 * This class extends the {@link SerialPort} class and functions in the same
 * way, however, there are a couple of differences to be aware of:
 * <ul>
 * <li>To "open" a serial port, create an instance of this class, configure it
 * as required, and then get the {@link TelnetClient} via
 * {@link #getTelnetClient} and invoke
 * {@link TelnetClient#connect(java.net.InetAddress, int)
 * TelnetClient.connect()} (or one of its variants). This will create the telnet
 * connection to the access server.</li>
 *
 * <li>Once connected, if the underlying telnet connection is broken, an
 * {@link IOException} will be thrown when attempting to access the serial port
 * input or output streams. In addition, a {@link SerialPortEvent#DATA_AVAILABLE
 * DATA_AVAILABLE} event will be immediately generated.</li>
 * </ul>
 * 
 *
 * 
 * The following optional functionality is not implemented and/or inappropriate
 * for a networked connection:
 * <ul>
 ...
 *
 * 
 * In addition, access servers typically don't support
 * {@link #notifyOnOutputEmpty}.
 * 
 *
 * 
 * Finally, {@link #sendBreak} is supported but the {@code millis} argument is
 * ignored, as timing cannot be assured over a TCP connection. Access servers
 * typically enforce a fixed break time.
 * 
 *
 * @see SerialPort
...
public class TelnetSerialPort extends SerialPort {
	...
	// Modem state bits we always want the server to report to us regardless of what
	// listener wants.
	// This is so we can always stay up-to-date with their values in case isCD(),
	// etc. is invoked.
	...
	// Line state bits we never want the server to report to us regardless of what
	// listener wants; internally,
	// we use the LINESTATE_DATA_READY bit only to indicate the listener wants
	// DATA_AVAILABLE notifications.
	private static final int LINESTATE_NEVER_MONITOR = LINESTATE_DATA_READY;

	// States
	private enum State {
		INITIAL(false, false), ESTABLISHED(true, false), CLOSED(false, true);
	...
	/**
	 * Get the signature sent to the remote server at connection time. By default,
	 * the signature is the name of this class.
	 */
	 ...
	 *
	 * @param signature
	 *            signature string, or {@code null} (or empty string) to not send a
	 *            signature
	 */
	...
	/**
	 * Construct and configure the {@link TelnetClient} to be used for this
	 * instance.
	 */
	...
	// We wrap the telnet port's InputStream in a NotifyInputStream so we can detect
	// when there
	// is new data available to be read. It would be nice if the TelnetClient
	// provided a way to
	...
	public void setFlowControlMode(int flowControl) throws UnsupportedCommOperationException {
		...
		synchronized (this) {
			...
			this.flowControlOutbound = (flowControl & FLOWCONTROL_RTSCTS_OUT) != 0
			...
			this.flowControlInbound = (flowControl & FLOWCONTROL_RTSCTS_IN) != 0
	...
	// This is invoked by the ComPortOptionHandler once the server has agreed to
	// accept COM-PORT-OPTION subnegotiation commands
	...
	// Method to send serial port "geometry" in the order recommended by RFC 2217
	// (section 2)
	...
	// This is invoked by the ComPortOptionHandler when we receive a command from
	// the server
	...
	// Update line state notifications; return true if we need to send new mask to
	// access server
	...
	// Update modem state notifications; return true if we need to send new mask to
	// access server
